...
export const handleStaffSignup = async (req: Request, res: Response) => {
    ...
    const identificationExists = await GetUserByIdentification(identification_card, role);
    if (identificationExists) {
      return res.status(400).json({ error: true, message: "There is another user with the given identification card" });
    }

    // Check if the mail already exists
    const mailExists = await GetUserByIdentification(mail, role);
    if (mailExists) {
      return res.status(400).json({ error: true, message: "There is another user with the given mail" });
    }
